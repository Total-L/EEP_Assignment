...
const Header: React.FC<HeaderProps> = ({ users, viewMode, setViewMode, onExport }) => {
    return (
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex -space-x-2">
                            {users.slice(0, 2).map(user => (
                                <img key={user.id} alt={user.name} className="w-8 h-8 rounded-full border-2 border-background-dark object-cover" src={user.avatarUrl} />
                            ))}
                            {users.length > 2 && <div className="w-8 h-8 rounded-full border-2 border-background-dark bg-slate-700 flex items-center justify-center text-[10px] font-bold">+{users.length - 2}</div>}
                        </div>
